...
class TestPdfAuth(unittest.TestCase):
    def setUp(self):
        ...
        self.valid_token = jwt.encode(
            {"id": 42, "username": "pdf_operator", "role": "operator", "token_version": 1, "exp": int(time.time()) + 3600},
            self.secret,
            algorithm="HS256"
        )
    ...
    def test_missing_exp_claim_rejection(self):
        token_no_exp = jwt.encode(
            {"id": 42, "username": "pdf_operator", "role": "operator", "token_version": 1},
            self.secret,
            algorithm="HS256"
        )
    ...
    def test_invalid_signature_rejection(self):
        tampered_token = jwt.encode(
            {"id": 42, "username": "hacker", "role": "admin", "token_version": 1, "exp": int(time.time()) + 3600},
    ...
    def test_missing_role_claim_rejection(self):
        token_no_role = jwt.encode(
            {"id": 42, "username": "no_role_user", "token_version": 1, "exp": int(time.time()) + 3600},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
    ...
    def test_must_change_password_forbidden(self):
        token_must_change = jwt.encode(
            {"id": 42, "username": "new_user", "role": "viewer", "token_version": 1, "must_change_password": True, "exp": int(time.time()) + 3600},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
    ...
    def test_asymmetric_rs256_verification(self):
        ...
        # Encode token using RS256 private key
        rs256_token = jwt.encode(
            {"id": 77, "username": "rsa_user", "role": "admin", "token_version": 1, "exp": int(time.time()) + 3600},
            priv_pem,
            algorithm="RS256"
        )

        # Set public key in environment
