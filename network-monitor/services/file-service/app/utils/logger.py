...
import base64
import pymssql
from app.config import LOG_MODE, LOGS_DIR, DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD
...
def ensure_db_table():
    ...
    try:
        ...
        cursor.execute("""
            IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'file_jobs')
            BEGIN
        ...
        print("File Service: Database logger table 'file_jobs' verified.", flush=True)
    except Exception as e:
        print(f"File Service: Database table initialization failed: {e}", flush=True)
...
def log_job(
    ...
    """
    Logs job details depending on FILE_TOOLS_LOG_MODE.
    """
    if LOG_MODE == "none":
        return
    ...
    if LOG_MODE == "jsonl":
        try:
            ...
            log_file = os.path.join(LOGS_DIR, f"file_tools_{today_str}.jsonl")
            ...
            print(f"File Service: Failed to write to JSONL log: {e}", flush=True)
            
    # 2. Log to Database
    elif LOG_MODE == "database":
        try:
            ensure_db_table()
            conn = get_db_connection()
            cursor = conn.cursor()
            ...
            print(f"File Service: Failed to log job to database: {e}", flush=True)
