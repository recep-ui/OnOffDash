...
class TestFileAuth(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_healthy_introspection.read.return_value = (
            b'{"active": true, "user": {"id": 99, "username": "file_operator", "role": "operator", "token_version": 1}}'
        )
    ...
    def test_missing_exp_claim_rejection(self):
        token_no_exp = jwt.encode(
            {"id": 99, "username": "file_operator", "role": "operator", "token_version": 1, "iat": int(time.time()), "iss": "onoffdash-auth", "aud": "onoffdash"},
            self.secret,
            algorithm="HS256"
        )
    ...
    def test_missing_token_version_rejection(self):
        token_no_tv = jwt.encode(
            {"id": 99, "username": "file_operator", "role": "operator", "exp": int(time.time()) + 3600, "iat": int(time.time()), "iss": "onoffdash-auth", "aud": "onoffdash"},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
    ...
    def test_invalid_signature_rejection(self):
        tampered_token = jwt.encode(
            {"id": 99, "username": "attacker", "role": "admin", "token_version": 1, "exp": int(time.time()) + 3600, "iat": int(time.time()), "iss": "onoffdash-auth", "aud": "onoffdash"},
    ...
    def test_wrong_issuer_rejection(self):
        wrong_iss_token = jwt.encode(
            {"id": 99, "username": "file_operator", "role": "operator", "token_version": 1, "exp": int(time.time()) + 3600, "iat": int(time.time()), "iss": "rogue-auth-service", "aud": "onoffdash"},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
    ...
    def test_wrong_audience_rejection(self):
        wrong_aud_token = jwt.encode(
            {"id": 99, "username": "file_operator", "role": "operator", "token_version": 1, "exp": int(time.time()) + 3600, "iat": int(time.time()), "iss": "onoffdash-auth", "aud": "rogue-service"},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
    ...
    def test_missing_iss_or_aud_rejection(self):
        no_iss_token = jwt.encode(
            {"id": 99, "username": "file_operator", "role": "operator", "token_version": 1, "exp": int(time.time()) + 3600, "iat": int(time.time())},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
    ...
    def test_must_change_password_forbidden(self):
        token_must_change = jwt.encode(
            {"id": 99, "username": "new_user", "role": "viewer", "token_version": 1, "must_change_password": True, "exp": int(time.time()) + 3600, "iat": int(time.time()), "iss": "onoffdash-auth", "aud": "onoffdash"},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
    ...
    def test_introspection_token_version_mismatch_rejection(self):
        ...
        mock_response.read.return_value = b'{"active": true, "user": {"id": 99, "role": "operator", "token_version": 2}}'
        mock_response.__enter__.return_value = mock_response
    ...
    def test_introspection_role_mismatch_rejection(self):
        ...
        mock_response.read.return_value = b'{"active": true, "user": {"id": 99, "role": "viewer", "token_version": 1}}'
        mock_response.__enter__.return_value = mock_response
    ...
    def test_introspection_must_change_password_forbidden(self):
        mock_response = MagicMock()
        mock_response.read.return_value = b'{"active": true, "user": {"id": 99, "role": "operator", "token_version": 1, "must_change_password": true}}'
        mock_response.__enter__.return_value = mock_response
    ...
    def test_asymmetric_rs256_verification(self):
        ...
        try:
            ...
            mock_rs256_resp.read.return_value = b'{"active": true, "user": {"id": 88, "username": "file_admin", "role": "admin", "token_version": 1}}'
