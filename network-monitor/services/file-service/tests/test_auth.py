...
class TestFileAuth(unittest.TestCase):
    ...
    def test_invalid_signature_rejection(self):
        tampered_token = jwt.encode(
            {"id": 99, "username": "attacker", "role": "admin", "exp": int(time.time()) + 3600},
    ...
    def test_missing_role_claim_rejection(self):
        token_no_role = jwt.encode(
            {"id": 99, "username": "no_role_user", "exp": int(time.time()) + 3600},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
    ...
    def test_must_change_password_forbidden(self):
        token_must_change = jwt.encode(
            {"id": 99, "username": "new_user", "role": "viewer", "must_change_password": True, "exp": int(time.time()) + 3600},
            self.secret,
            algorithm="HS256"
        )
        with self.assertRaises(HTTPException) as ctx:
